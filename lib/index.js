...
async function handleApi(ctx, req, res, url, config) {
  // 宿主 apiProxy 的每个方法都要求有效 AbortSignal（内部直接
  // signal.addEventListener，不防御 undefined）。Node 的 IncomingMessage
  // 没有 signal 属性（req.signal 恒为 undefined），这里为本次请求创建
  // 真实取消信号：响应关闭（含客户端断开）时 abort，流/导出随之终止。
  ...
  if ((pathname === '/api/events.mux' || pathname === '/api/events.host') && req.method === 'GET') {
    ...
    try {
      ...
      return await pipeSse(stream, method, res, signal)
    } catch (error) {
  ...
  if (pathname === '/api/session.export' && (req.method === 'GET' || req.method === 'HEAD')) {
    try {
      ...
      const response = await api.downloads.sessionLog({ sessionId }, signal)
      if (req.method === 'HEAD') {
